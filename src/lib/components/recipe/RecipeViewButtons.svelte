<script>
	...
	async function confirmDuplicate() {
		...
		isCopying = true
		const result = await duplicateRecipe(recipe?.uid)
		isCopying = false
		if (!result.success || !result.uid) return
		await invalidateAll()
		goto(`/recipe/${result.uid}/view/`)
	}
</script>
...
{:else}
	<button class="btn btn-outline btn-sm tooltip" data-tip={$t('recipe.privateLabel')}>
...
{#if !viewOnly}
	...
	<button
		...
		data-tip={logs?.length > 0
			? $t(logs.length === 1 ? 'recipe.cookedTimes_one' : 'recipe.cookedTimes_other', {
					count: logs.length
				})
			: $t('recipe.markCooked')}
