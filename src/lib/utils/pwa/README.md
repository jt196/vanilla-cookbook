...
2. In _entrypoint.sh_, `npm serve` will run the replacement script: `./scripts/pwa/sw-domain.sh`. This will depend on the local deployment environment, and replaces the `%%URLPATTERN%%` in the generated service-worker files.
